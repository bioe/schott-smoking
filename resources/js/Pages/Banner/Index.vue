...
const destroy = (id, name) => {
    const c = confirm(`Delete this banner ${name} ?`);
    if (c) {
                ...
                <tbody>
                    <tr v-for="(item, index) in list.data">
                        <td width="10%">
                            ...
                            <button @click="destroy(item.id, item.title)" class="btn btn-sm btn-link">
                                <i class="bi bi-trash"></i>
                            </button>
                        </td>
                        <td :class="item.active == 0 ? 'text-danger' : ''">{{ item.title }}</td>
                        <td><a :href="item.full_path" target="_blank">{{ item.filename }}</a></td>
